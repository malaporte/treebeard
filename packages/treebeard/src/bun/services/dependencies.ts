...
type DependencyName = DependencyCheck['name']
...
async function checkDependency(name: DependencyName, env: Record<string, string>): Promise<DependencyCheck> {
  const commandProbes = name === 'jira'
    ? [['--version'], ['version']]
    : [['--version']]
